 ...
 * @author Paul Schaub
 */
public abstract class OmemoServiceOld<T_IdKeyPair, T_IdKey, T_PreKey, T_SigPreKey, T_Sess, T_Addr, T_ECPub, T_Bundle, T_Ciph>
        implements OmemoCarbonCopyStanzaReceivedListener, OmemoMessageStanzaReceivedListener
{
     ...
     * This is a singleton.
     */
    private static OmemoServiceOld<?, ?, ?, ?, ?, ?, ?, ?, ?> INSTANCE;
     ...
     * @return instance.
     */
    public static OmemoServiceOld<?, ?, ?, ?, ?, ?, ?, ?, ?> getInstance() {
        if (INSTANCE == null) {
     ...
     */
    protected static void setInstance(OmemoServiceOld<?, ?, ?, ?, ?, ?, ?, ?, ?> omemoService) {
        if (INSTANCE != null) {
            ...
            PubSubException.NotALeafNodeException
    {
        ...
        OmemoCachedDeviceList list = refreshOwnDeviceList(managerGuard);
    ...
    static OmemoDeviceListElement fetchDeviceList(OmemoManager.LoggedInOmemoManager managerGuard, BareJid contact)
        ...
        OmemoManager omemoManager = managerGuard.get();
        try {
            LeafNode node = PubSubManager.getInstance(omemoManager.getConnection(), contact).getLeafNode(PEP_NODE_DEVICE_LIST);
     ...
     * @throws SmackException.NoResponseException
     */
    private OmemoCachedDeviceList refreshOwnDeviceList(OmemoManager.LoggedInOmemoManager managerGuard)
            throws SmackException.NotConnectedException, InterruptedException, SmackException.NoResponseException,
            XMPPException.XMPPErrorException
    {
        return refreshDeviceList(managerGuard, managerGuard.get().getOwnJid());
    }
     ...
     * @throws SmackException.NoResponseException
     */
    OmemoCachedDeviceList refreshDeviceList(OmemoManager.LoggedInOmemoManager managerGuard, BareJid contact)
            throws SmackException.NotConnectedException, InterruptedException, SmackException.NoResponseException
    {
        OmemoManager omemoManager = managerGuard.get();
        OmemoDevice userDevice = omemoManager.getOwnDevice();

        OmemoDeviceListElement list = null;
        try {
            list = fetchDeviceList(managerGuard, contact);
        } catch (XMPPException.XMPPErrorException e) {
            LOGGER.log(Level.WARNING, "Could not refresh DeviceList of " + contact + ".", e);
        }
        return getOmemoStoreBackend().mergeCachedDeviceList(userDevice, contact, list);
    }
    ...
    void buildMissingOmemoSessions(OmemoManager.LoggedInOmemoManager managerGuard, BareJid jid)
        ...
        OmemoCachedDeviceList devices = getOmemoStoreBackend().loadCachedDeviceList(userDevice, jid);
            ...
            CannotEstablishOmemoSessionException
    {
        ...
        for (BareJid recipient : recipients) {
            OmemoCachedDeviceList theirDevices = getOmemoStoreBackend().loadCachedDeviceList(userDevice, recipient);
        ...
        OmemoCachedDeviceList ourDevices = getOmemoStoreBackend().loadCachedDeviceList(userDevice, omemoManager.getOwnJid());
        if (ourDevices == null) {
            ourDevices = new OmemoCachedDeviceList();
        }
